...
def scrape_products(url):
    ...
    # where there's no display to show a real browser on. The legacy
    # "--headless" flag is used here rather than "--headless=new",
    # since the newer mode has known stability issues (intermittent
    # session/navigation errors) on some Chrome/driver version
    # combinations, particularly in constrained container environments.
    ...
    # Containers typically allocate a very small /dev/shm (shared
    # memory), which Chrome's renderer process relies on heavily by
    # default. When it runs out, the renderer can crash silently,
    # leaving the browser window gone before we're done using it. This
    # flag makes Chrome fall back to disk-based temp files instead.
